...
#
# Modified script by Massimo Tamos:
#
# 1: To run standalone without a docker container
# 2: Enables api key authentication
...
def print_rule(action, alert_list, detailed_output, user_msg):
    ...
    print (action + ': ' + alert_list[0].get('alert') + ' [' + alert_list[0].get('pluginId') + '] x ' + str(
...
def main(argv):
    ...
    try:
        ...
        spider_scan_id = zap.spider.scan(target,apikey=api_key)
        time.sleep(5)
        ...
        if (ajax):
            ...
            zap.ajaxSpider.scan(target,apikey=api_key)
            time.sleep(5)
        ...
        while (int(zap.pscan.records_to_scan) > 0):
            logging.info('Records to passive scan : ' + zap.pscan.records_to_scan)
            time.sleep(2)
        logging.info('Passive scanning complete')

        if (active_scan):
            logging.info('Active Scanning target: ' + target)
            spider_scan_id = zap.ascan.scan(target, apikey=api_key)
            while (int(zap.ascan.status(spider_scan_id)) < 100):
               logging.info('Scan progress: ' + zap.ascan.status(spider_scan_id))
               time.sleep(5)
        ...
        if (num_urls == 0):
            ...
            if detailed_output:
                logging.info ('Total of ' + str(len(zap.core.urls)) + ' URLs')
            ...
            for key, alert_list in sorted(alert_dict.iteritems()):
                if (config_dict.has_key(key) and config_dict[key] == 'INFO') or (
                not config_dict.has_key(key)) and info_unspecified:
                    user_msg = ''
                    if key in config_msg:
                        user_msg = config_msg[key]
            ...
            for key, alert_list in sorted(alert_dict.iteritems()):
                if (not config_dict.has_key(key) and not info_unspecified) or (
                    config_dict.has_key(key) and config_dict[key] == 'WARN'):
                    user_msg = ''
                    if key in config_msg:
                        user_msg = config_msg[key]
